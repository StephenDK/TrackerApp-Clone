...
function PhoneInventoryCheckout(props) {
  // State
  ...
  const onFormSubmit = async () => {
    try {
      ...
      let req = await axios.post(
        `${config.apiUrl}/api/v1/phones/inventory/checkout`,
        data
      );
      if (req.status === 200) {
  ...
  const validateSerial = (input) => {
    // REGEX pattern check imei
    const blacklistedChars = /^\d{15}$/;
  ...
  const validateUserName = (input) => {
    // REGEX pattern check for username
          ...
          <Grid container spacing={3} rowSpacing={3}>
            <Grid item xs={12} sm={12}>
              <TextField
                ...
                fullWidth
                helperText={
                  validateInput.invalidSerial ? "Incorrect IMEI format" : null
                }
              ...
              <TextField
                ...
                helperText={
                  validateInput.invalidUser
                    ? "Username format must be first.last"
                    : null
                }

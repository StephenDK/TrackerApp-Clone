...
function MonitorView({ files, loading, nav }) {
  // State
  const { id } = useParams();
  ...
  const componentMapping = {
    Icon: { component: <MonitorIcon sx={{ fontSize: 40 }} /> },
  };
  ...
  useEffect(() => {
    let initializePage = async () => {
      try {
        ...
        let request = await axios.get(`${config.apiUrl}/api/v1/monitors/${id}`);
  ...
  return (
    <Fragment>
      ...
      <DialogBox toggleDialog={nav.itemViews.toggleDelete}>
        <DeleteForm API={"monitors"} ID={data._id} />
      </DialogBox>
    </Fragment>
  );
}

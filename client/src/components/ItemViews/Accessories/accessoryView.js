...
import {} from "../../../actions/filesActions";
...
function AccessoryView({ files, loading, nav }) {
  // State
  const { id } = useParams();
  ...
  const componentMapping = {
    Icon: { component: <CableIcon sx={{ fontSize: 40 }} /> },
  };
    ...
    let initializePage = async () => {
      try {
        ...
        let request = await axios.get(
          `${config.apiUrl}/api/v1/accessory/${id}`
        );
    ...
    <Fragment>
      ...
      <DialogBox toggleDialog={nav.itemViews.toggleCheckOut}>
        <CheckOutForm
          updateUI={setData}
          API={"accessory"}
          textHeading={"Check Out Accessory"}
          icon={componentMapping}
          ID={data._id}
      ...
      <DialogBox toggleDialog={nav.itemViews.toggleCheckIn}>
        <CheckInForm
          ...
          API={"accessory"}
          ID={data._id}
          textHeading={"Check In Accessory"}
          icon={componentMapping}
        />
      </DialogBox>
      ...
      <DialogBox toggleDialog={nav.itemViews.toggleDelete}>
        <DeleteForm API={"accessory"} ID={data._id} />
      </DialogBox>
    </Fragment>
  );
}
